...
void save_svg(const std::vector<Polygon> &polygons, std::string filename, std::string fillcol = "none")
{
	...
	{
		if (polygons[i].get_n_vertices() > 0)
		{
			fprintf(f, "<g>\n");
			fprintf(f, "<polygon points = \"");
			for (int j = 0; j < polygons[i].get_n_vertices(); j++)
			{
				fprintf(f, "%3.3f, %3.3f ", (polygons[i].get_vertice(j)[0] * 1000), (1000 - polygons[i].get_vertice(j)[1] * 1000));
			}
			fprintf(f, "\"\nfill = \"%s\" stroke = \"black\"/>\n", fillcol.c_str());
			fprintf(f, "</g>\n");
		}
